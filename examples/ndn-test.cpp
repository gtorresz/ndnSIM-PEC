...
main(int argc, char* argv[])
{
  ...
  ndn::StrategyChoiceHelper::Install(nodes.Get( 0 ),"/prefix", "/localhost/nfd/strategy/intel");

  // Installing applications

  // Consumer
  ndn::AppHelper consumerHelper("ns3::ndn::PECServer");
  // Consumer will request /prefix/0, /prefix/1, ...
  consumerHelper.SetPrefix("/prefix/server1");
...
//  consumerHelper.SetAttribute( "Subscription", IntegerValue( 1 ) );
  ...
  consumerHelper.SetPrefix("/prefix/server2");
  consumerHelper.Install(nodes.Get(3));
  consumerHelper.SetPrefix("/prefix/server3");
  consumerHelper.Install(nodes.Get(4));
//  apps.Stop(Seconds(10.0)); // stop the consumer app at 10 seconds mark


  // Producer
  ndn::AppHelper producerHelper("ns3::ndn::BaseStation");
